...
def page_resources():
    ...
    for r in st.session_state.resources.get("resources", []):
        ...
        search_query = r.get('url_hint', r['title']).replace(' ', '+')
        ...
        link_section = ""
        if r["type"] == "Video":
            link_section = f'<a href="{youtube_link}" target="_blank" style="color:var(--accent2);text-decoration:none;">▶ Search on YouTube</a>'
        else:
            link_section = f'<a href="{google_link}" target="_blank" style="color:var(--accent);text-decoration:none;">🔗 Find this resource</a>'
        st.markdown(f"""
<div class="resource-card">
  <h4>{icon} {r['title']} <span class="badge">{r['type']}</span> <span class="badge badge-accent">{r.get('platform','')}</span></h4>
  <p>{r['description']}</p>
  <small style="color:var(--muted)">⏱ {r.get('duration','')}</small><br>
  {link_section}
</div>
""", unsafe_allow_html=True)
